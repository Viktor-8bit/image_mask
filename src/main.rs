...
fn _color_calc(x: u32, y: u32, index: usize, img: &DynamicImage) -> u8 {
    let mut result: i32 = img.get_pixel(x, y)[index] as i32 * _MASK[1][1]
        + img.get_pixel(x + 1, y)[index] as i32 * _MASK[2][1] 
        + img.get_pixel(x, y + 1)[index] as i32 * _MASK[1][2] 
        + img.get_pixel(x - 1, y)[index] as i32 * _MASK[0][1]
        + img.get_pixel(x, y - 1)[index] as i32 * _MASK[1][0]
        + img.get_pixel(x - 1, y - 1)[index] as i32 * _MASK[0][0] 
        + img.get_pixel(x + 1, y + 1)[index] as i32 * _MASK[2][2]
        + img.get_pixel(x - 1, y + 1)[index] as i32 * _MASK[0][2] 
        + img.get_pixel(x + 1, y - 1)[index] as i32 * _MASK[2][0];
